...
// each line is x1 y1 x2 y2   color: Scalar(255,0,0) eg
void plot_image_with_edges(const cv::Mat& rgb_img, cv::Mat& output_img, vector<cv::line_descriptor::KeyLine>& all_lines, const cv::Scalar& color)
{
    output_img = rgb_img.clone();
    for (int i = 0; i < all_lines.size(); i++)
        cv::line(   output_img,
                    cv::Point(all_lines[i].startPointX,all_lines[i].startPointY),
                    cv::Point(all_lines[i].endPointX,all_lines[i].endPointY), 
                    color, 
                    2, 
                    16, 
                    0);
...
bool check_inside_box(const cv::line_descriptor::KeyLine& pt, const Vector2d& box_left_top, const Vector2d& box_right_bottom)
{
    bool check = box_left_top(0)<=pt.startPointX && pt.startPointX<=box_right_bottom(0) && box_left_top(1)<=pt.startPointY && pt.startPointY<=box_right_bottom(1);
    check &= box_left_top(0)<=pt.endPointX && pt.endPointX<=box_right_bottom(0) && box_left_top(1)<=pt.endPointY && pt.endPointY<=box_right_bottom(1);
    return check;
}
...
void align_left_right_edges(std::vector<cv::line_descriptor::KeyLine> & all_lines)
{
    for (int line_id=0; line_id < all_lines.size(); line_id++)
    {
        // 0 1， 2 3 要求第2个点的x坐标（2）要大于第一个点的 x 坐标（0）.
        if (all_lines[line_id].endPointX < all_lines[line_id].startPointX)
        {
            float stx = all_lines[line_id].startPointX, sty = all_lines[line_id].startPointY;
            all_lines[line_id].startPointX = all_lines[line_id].endPointX;
            all_lines[line_id].startPointY = all_lines[line_id].endPointY;
            all_lines[line_id].endPointX = stx;
            all_lines[line_id].endPointY = sty;
        }
    }
}
