...
detect_3d_cuboid::detect_3d_cuboid(float fx,float fy,float cx,float cy)
{
	Eigen::Matrix3d Kalib;
	Kalib  <<  721.5377,  0,  609.5593,
                0,  721.5377,  172.854,
                0,         0,    1.0000;
    set_calibration(Kalib);
										...
										std::vector<ObjectSet>& all_object_cuboids)
{
	...
	// cv::Mat temp_img = rgb_img.clone();
	// for(int i=0;i<obj_bbox_coors.size();i++)
	// {
	// 	cv::rectangle(temp_img,obj_bbox_coors[i],cv::Scalar(255,0,0),1);
	// }
	// cv::imshow("rect",temp_img);
	// cv::waitKey(0);
	// 绘制上边缘采样点
	typedef cv::Point_<int> Point2i;
	std::vector<Point2i> simple_points;	
	cv::Mat image_point = rgb_img.clone();	
	...
	// 读取检测框信息的行数，也即图像帧数.
    int num_2d_objs = obj_bbox_coors.size();
    ...
    for (int object_id = 0; object_id < num_2d_objs; object_id++)
    {
		...
		int left_x_raw = max(0,obj_bbox_coors[object_id].x-5); 
		int top_y_raw = max(0,obj_bbox_coors[object_id].y-5); 
		int obj_width_raw = obj_bbox_coors[object_id].width+10; 
		int obj_height_raw = obj_bbox_coors[object_id].height+10;
		// 计算右下角点的坐标.
		int right_x_raw = left_x_raw + obj_width_raw; 
		int down_y_raw = top_y_raw + obj_height_raw;
		if(right_x_raw>=rgb_img.cols){
			right_x_raw = rgb_img.cols -1;
			obj_width_raw = right_x_raw - left_x_raw;
		}
		if(down_y_raw>=rgb_img.rows){
			down_y_raw = rgb_img.rows -1;
			obj_height_raw = down_y_raw - top_y_raw;
		}
		// NOTE 绘制检测框.
		rectangle(	rgb_img,
		...
		for (int sample_down_expan_id = 0; sample_down_expan_id < down_expand_sample_all.size(); sample_down_expan_id++)
		{
			...
			int top_sample_resolution = round(min(20,obj_width_raw/20 )); //  25 pixels
			...
			// @PARAM 线段合并与筛选参数.
			double pre_merge_dist_thre = 5; 
			double pre_merge_angle_thre = 3; 
			...
			// 显示筛选之后的边缘线段.
			// cv::Mat output_img;
			// plot_image_with_edges(rgb_img, output_img, all_lines_merge_inobj, cv::Scalar(0,255,0));
			// cv::namedWindow("merge_lines_img");
			// cv::moveWindow("merge_lines_img",500, 300);
			...
			// NOTE Canny 算子边缘检测.
			cv::Mat im_canny; 
			cv::Canny(gray_img(object_bbox),im_canny,40,100); // low thre, high thre    im_canny 0 or 255   [80 200  40 100]
			// 距离变换.
			cv::Mat dist_map; 
