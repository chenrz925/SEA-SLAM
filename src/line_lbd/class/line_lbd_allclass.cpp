...
{  
    ...
    cout<<keylines_out.size()<<endl;
}
void line_lbd_detect::filter_lines(std::vector< KeyLine>& keylines_in,Eigen::MatrixXd& keylines_out)
{  
    // 删除短线.

    if (keylines_out.rows()==0)
	    keylines_out.resize(100, 10);  
    int row_counter = 0;
    for ( int i = 0; i < (int) keylines_in.size(); i++ )   // keep octave 0, and remove short lines
        if( keylines_in[i].octave == 0 )
            if (keylines_in[i].lineLength>line_length_thres){
                keylines_out(row_counter,0) = keylines_in[i].startPointX;
                keylines_out(row_counter,1) = keylines_in[i].startPointY;
                keylines_out(row_counter,2) = keylines_in[i].endPointX;
                keylines_out(row_counter,3) = keylines_in[i].endPointY;
                row_counter++;
                if (row_counter>=keylines_out.rows()) // if matrix row is not enough, make more space.
                    keylines_out.conservativeResize(keylines_out.rows()*2,keylines_out.cols());
            }
