...
int main(int argc, char **argv)
{
    ...
    {
        cerr << endl
             << "Usage: rosrun ORB_SLAM3 Stereo_Inertial path_to_vocabulary path_to_settings do_rectify [do_equalize] [saving_path]" << endl;
        ros::shutdown();
        return 1;
    }
    ...
    ImuGrabber imugb;
    ImageGrabber igb(&SLAM, &imugb,sbRect == "true", bEqual); // TODO
    if(igb.do_rectify)
    {      
    ...
    ros::Subscriber sub_img_left = nh.subscribe("/camera/left/image_raw", 100, &ImageGrabber::GrabImageLeft,&igb);
    ros::Subscriber sub_img_right = nh.subscribe("/camera/right/image_raw", 100, &ImageGrabber::GrabImageRight,&igb);
