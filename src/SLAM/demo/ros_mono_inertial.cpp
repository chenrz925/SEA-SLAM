...
void ImageGrabber::SyncWithImu()
{
    ...
    ORB_SLAM3::Semantic::GetInstance()->RequestFinish();

    // Stop all threads
    mpSLAM->Shutdown();
    mpSLAM->SaveTrajectoryTUM(mpSLAM->mstrSavingPath+"/CameraTrajectory.txt");
    mpSLAM->SaveKeyFrameTrajectoryTUM(mpSLAM->mstrSavingPath+"/KeyFrameTrajectory.txt");
...
int main(int argc, char **argv)
{
    ...
    {
        cerr << endl
             << "Usage: rosrun ORB_SLAM3 Mono_Inertial path_to_vocabulary path_to_settings [do_equalize] saving_path" << endl;
        ros::shutdown();
        return 1;
    }
    ...
    if (argc == 4)
    {
        ...
        ORB_SLAM3::Config::GetInstance()->createSavePath(string(argv[5]));
    }
    else{
    ...
    cout << "argv[5] result path: " << argv[4] << endl;
    cout << "===========================" << endl;
    ...
    ros::Subscriber sub_img0 = nh.subscribe("/image", 100, &ImageGrabber::GrabImage, &igb);
