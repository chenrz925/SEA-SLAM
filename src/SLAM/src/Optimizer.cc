...
using namespace cv;
namespace ORB_SLAM3 {
WeightCalculation* Optimizer::mWeightCalcu = new WeightCalculation();
...
void ThreeMaxima(vector<float> histo, const int L, int& ind1, int& ind2, int& ind3)
{
    int max1 = 0;
    int max2 = 0;
    int max3 = 0;
...
void Optimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    // TODO only use keyframes that has semantic
    // for (std::vector<KeyFrame*>::iterator it = vpKFs.begin(); it != vpKFs.end(); it++) {
    //     KeyFrame* pKF = *it;
    //     if (!pKF->IsSemanticReady()) {
    //         it = vpKFs.erase(it);
    //         continue;
    //     }
    // }
    for (std::vector<MapPoint*>::iterator it = vpMP.begin(); it != vpMP.end(); it++) {
        MapPoint* pMP = *it;
...
void Optimizer::GlobalBundleAdjustemntForInitialMono(Map* pMap, int nIterations,bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
...
int Optimizer::PoseOptimization(Frame* pFrame)
{
    ...
    int outlier_map_mp = 0;
    {
        ...
        for (int i = 0; i < N; i++) {
            ...
            // if(pFrame->mvDepth[i]>6) 
            if(pFrame->mvbOutlier[i] )
                continue;
            if (pMP) {
                // if(pFrame->mvpCluster[i])   continue;
                // TODO [Semantic] Skip outliers according Moving probability
                if (pMP->IsDynamicMapPoint()||!pFrame->checkNeighbor(i)) {
                    outlier_map_mp++;
    ...
    const bool bFixScale, Eigen::Matrix<double, 7, 7>& mAcumHessian, const bool bAllPoints)
{
    ...
    cv::Mat img1,img2;
    if (bShowImages){
        img1 = cv::imread(pKF1->mNameFile, IMREAD_UNCHANGED);
        cv::cvtColor(img1, img1, CV_GRAY2BGR);
        img2 = cv::imread(pKF2->mNameFile, IMREAD_UNCHANGED);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF, vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, bool* pbStopFlag)
{
    ...
    for (KeyFrame* pKFi : vpFixedKF) {
        ...
        // TODO skip keyframes that no semantic info
        // if (!pKFi->IsSemanticReady()) {
        //     continue;
        // }
        ...
        for (MapPoint* pMPi : spViewMPs) {
            if (pMPi)
                if (!pMPi->isBad() && pMPi->GetMap() == pCurrentMap) {
                    // TODO semantic
                    if (pMPi->GetMovingProbability() >= 0.5) {
                        continue;
                    }
    ...
    for (KeyFrame* pKFi : vpAdjustKF) {
        ...
        for (MapPoint* pMPi : spViewMPs) {
            if (pMPi) {
                if (!pMPi->isBad() && pMPi->GetMap() == pCurrentMap) {
                    // if (Semantic::GetInstance()->IsDynamicMapPoint(pMPi)) {
                    //     continue;
                    // }
                    // TODO semantic
                    if (pMPi->GetMovingProbability() >= 0.5) {
...
int Optimizer::PoseOptimization(KeyFrame* pFrame, bool bUpdateMap)
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (pMP) {
            ...
            //     continue;
            // }
            cv::Point kp = pFrame->mvKeys[i].pt;
            if(pFrame->mImMask.at<uchar>(kp.y,kp.x)!=0)
                continue;
            if(pMP->IsDynamicMapPoint())
                continue;
...
void Optimizer::ObjectOptimization(Frame* pLastFrame,Frame* pFrame,Cluster* pCluster)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType* linearSolver;
    ...
    // Set Frame vertex
    g2o::VertexSE3Expmap* vSE3 = new g2o::VertexSE3Expmap();
    vSE3->setEstimate(Converter::toSE3Quat(pCluster->mTow));
    vSE3->setId(0);
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);
    ...
    vector<ORB_SLAM3::EdgeProjectXYZPoseOnly*> vpEdgesMono;
    // vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody*> vpEdgesMono_FHR;
    vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight;
    vpEdgesMono.reserve(N);
    // vpEdgesMono_FHR.reserve(N);
    ...
    const float deltaMono = sqrt(5.991);
    const float deltaStereo = sqrt(7.815);
    vector<bool> vbOutliers = vector<bool>(N,false);
    int outlier_map_mp = 0;

    {
        ...
        for (int i = 0; i < N; i++) {
            MapPoint* pMP = pFrame->mvpClusterMapPoint[i];//pCluster->mvpMapPoint[i];
            if (pMP) {
                ...
                if (pFrame->mvuRight[i] < 0) {
                    nInitialCorrespondences++;
                    kpUn = pFrame->mvKeys[i];
                    cv::Mat lastPoint = pMP->GetWorldPos();
                    cv::Mat curPoint = pFrame->UnprojectStereo(i);
                    if(curPoint.empty())
                        continue;
                    // cv::Mat lastPoint = pLastFrame->mvpClusterMapPoint[pFrame->mvMatchId[i]]->GetWorldPos();
                    vbOutliers[i] = false;

                    ORB_SLAM3::EdgeProjectXYZPoseOnly* e = new ORB_SLAM3::EdgeProjectXYZPoseOnly(
                        Eigen::Vector3d(lastPoint.at<float>(0),lastPoint.at<float>(1),lastPoint.at<float>(2))
                    );

                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
                    e->setMeasurement(Eigen::Vector3d(curPoint.at<float>(0),curPoint.at<float>(1),curPoint.at<float>(2)));
                    const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                    e->setInformation(Eigen::Matrix3d::Identity() * invSigma2);
    ...
    for (size_t it = 0; it < 4; it++) {
        ...
        nBad = 0;
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ORB_SLAM3::EdgeProjectXYZPoseOnly* e = vpEdgesMono[i];
    ...
    g2o::SE3Quat SE3quat_recov = vSE3_recov->estimate();
    cv::Mat pose = Converter::toCvMat(SE3quat_recov);
    pCluster->SetPose(pose);
    
    for (int i = 0; i < pFrame->N; i++) {
        if (pFrame->mvpCluster[i]) {
            if (vbOutliers[i]) {
                MapPoint* pMP = pFrame->mvpClusterMapPoint[i];
                pFrame->mvpCluster[i] = static_cast<Cluster*>(NULL);
                pCluster->EraseMapPoint(pMP);
                delete pMP;
            }
        }
    }
...
int Optimizer::PoseOptimizationWithRelativeFeature(Frame* pFrame)
{
        // vector<float> staticProb;
    // future<cv::Mat> getStaticProb = async(launch::async, [this] {
    // vector<float> staticProb = vector<float>(pFrame->N,0);
    // if(pFrame->mImMask.rows==0)
    //     return -1;
    // vector<pair<int,MapPoint*>> x3D;
    vector<pair<int,MapPoint*>> x3D_static;
    vector<pair<float, int> > vDepthIdx;
    vDepthIdx.reserve(pFrame->N);

    {
        ...
        for(int i=0;i<pFrame->N;i++)
        {
            ...
            MapPoint* pMP = pFrame->mvpMapPoints[i];
            if (pMP) {
                if(!pFrame->mImMask.empty())
                {
                    if(pFrame->mImMask.at<uchar>(pFrame->mvKeys[i].pt.y,pFrame->mvKeys[i].pt.x)==0){
                        // vDepthIdx.push_back(make_pair(z, num_id++));
                        x3D_static.push_back(make_pair(i,pFrame->mvpMapPoints[i]));
                    }
                ...
                // x3D.push_back(make_pair(i,pFrame->mvpMapPoints[i]));
            }
        }
    }
    // if (!vDepthIdx.empty()) 
    //     sort(vDepthIdx.begin(), vDepthIdx.end());
    vector<float> histDis = vector<float>(55,0);
    // cv::Mat showImg = pFrame->mImRGB.clone();
    int cnt_edge=0;
    int static_sz=x3D_static.size();
    vector<relativeEdge> optimize_edge(static_sz*10);
    // for(int i=0;i<x3D.size();i++)
    for(int i=0;i<static_sz;i++)
    {
        // cv::Mat cur3Di = pFrame->UnprojectStereoCamera(x3D[i].first);
        cv::Mat cur3Di = pFrame->UnprojectStereoCamera(x3D_static[i].first);
        if(cur3Di.rows==0) continue;
        cv::Mat cur3Dk ;
        // cv::Mat posi=x3D[i].second->GetWorldPos();
        cv::Mat posi=x3D_static[i].second->GetWorldPos();
        // for(int k=0;k<min(10,static_sz);k++)
        // {   
        //     int j=vDepthIdx[k].second;
        //     cur3Dk = pFrame->UnprojectStereoCamera(x3D_static[j].first);
        //     if(cur3Dk.rows==0){ continue;}
        //     if(x3D[i].first==x3D_static[j].first) {continue;}
        //     // cv::Mat dis_cur = cur3Di-cur3Dk;
        //     cv::Mat dis_cur = -cur3Di+2*cur3Dk;
        //     Eigen::Matrix<double, 3, 1> obs;
        //     obs<< dis_cur.at<float>(0),dis_cur.at<float>(1),dis_cur.at<float>(2);
        //     const float invSigma2 = pFrame->mvInvLevelSigma2[pFrame->mvKeysUn[x3D_static[j].first].octave];
        //     cv::Mat posk = x3D_static[j].second->GetWorldPos();
        //     // cv::Mat dis_ori = x3D[i].second->GetWorldPos()-x3D_static[k].second->GetWorldPos();
        //     cv::Mat dis_ori = -posi+2*posk;
        //     cv::Mat dis_temp_ori = posi-posk;
        //     cv::Mat dis_temp = pFrame->UnprojectStereo(x3D[i].first)-pFrame->UnprojectStereo(x3D_static[j].first);
        //     int disik = int(norm(dis_temp-dis_temp_ori)*10);
        //     if(disik>50) continue;
        //     histDis[disik]++;
        //     relativeEdge temp_edge;
        //     temp_edge.i=x3D[i].first;
        //     temp_edge.k=x3D_static[j].first;
        //     temp_edge.dis=disik;
        //     temp_edge.dis_cur=dis_cur.clone();
        //     temp_edge.dis_ori=dis_ori.clone();
        //     temp_edge.invSigma2 = invSigma2;
        //     temp_edge.movingProb = pFrame->mvpMapPoints[x3D_static[j].first]->GetMovingProbability();
        //     optimize_edge[cnt_edge]=temp_edge;
        //     cnt_edge++;
            
        for(int j=0;j<10;j++){
            int k = select_randomly(x3D_static.begin(), x3D_static.end())-x3D_static.begin();
            // int k=j;
            if(k==i) continue;
            cur3Dk = pFrame->UnprojectStereoCamera(x3D_static[k].first);
            if(cur3Dk.rows==0) continue;
            // if(x3D_static[k].first==x3D_static[i].first) continue;
            // cv::Mat dis_cur = cur3Di-cur3Dk;
            cv::Mat dis_cur = -cur3Di+2*cur3Dk;
            Eigen::Matrix<double, 3, 1> obs;
            obs<< dis_cur.at<float>(0),dis_cur.at<float>(1),dis_cur.at<float>(2);
            const float invSigma2 = pFrame->mvInvLevelSigma2[pFrame->mvKeysUn[x3D_static[k].first].octave];

            // cv::Mat dis_ori = x3D[i].second->GetWorldPos()-x3D_static[k].second->GetWorldPos();
            cv::Mat dis_ori = -posi+2*x3D_static[k].second->GetWorldPos();
            cv::Mat dis_temp_ori = posi-x3D_static[k].second->GetWorldPos();
            // cv::Mat dis_temp = pFrame->UnprojectStereo(x3D[i].first)-pFrame->UnprojectStereo(x3D_static[k].first);
            cv::Mat dis_temp = pFrame->UnprojectStereo(x3D_static[i].first)-pFrame->UnprojectStereo(x3D_static[k].first);
            int disik = int(norm(dis_temp-dis_temp_ori)*10);
            if(disik>3) continue;
            histDis[disik]++;
            relativeEdge temp_edge;
            temp_edge.i=x3D_static[i].first;
            temp_edge.k=x3D_static[k].first;
            temp_edge.dis=disik;
            temp_edge.dis_cur=dis_cur.clone();
            temp_edge.dis_ori=dis_ori.clone();
            temp_edge.invSigma2 = invSigma2;
            temp_edge.unstableProb =min(1.,norm(dis_temp-dis_temp_ori)); //x3D_static[k].second->GetMovingProbability();
            optimize_edge[cnt_edge]=temp_edge;
            cnt_edge++;
            
            // cv::circle(showImg,pFrame->mvKeys[x3D_static[k].first].pt,disik/2,cv::Scalar(0,0,255),-1);
        }
		// cv::imwrite("/media/mxc/bs/SeaSLAM/result/debug/relative_img.png",showImg);
    }
    // int dis1=-1,dis2=-1,dis3=-1;
    // ThreeMaxima(histDis,50,dis1,dis2,dis3);

    if(cnt_edge<30) return 0;
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType* linearSolver;
    ...
    vector<ORB_SLAM3::EdgeSE3RelativeXYZOnlyPose*> vpEdgesMono;
    vector<size_t> vnIndexEdgeMono1,vnIndexEdgeMono2, vnIndexEdgeRight;
    vpEdgesMono.reserve(N);
    vnIndexEdgeMono1.reserve(N);
    vnIndexEdgeMono2.reserve(N);
    const float deltaMono = sqrt(5.991);
    const float deltaStereo = sqrt(7.815);
    vector<pair<cv::Mat,cv::Mat> > allEdges;

    // vector<int> errHist[20];
    // for (int i = 0; i < 20; i++)
    //     errHist[i].reserve(500);
    // const float factor = 1.0f / 20;
    ...
    int outlier_map_mp = 0;
    {
        ...
        {
            relativeEdge temp=optimize_edge[i];
            if(temp.dis_cur.rows==0)    continue;
            // if(temp.dis==dis1||temp.dis==dis2||temp.dis==dis3)
            {
                Eigen::Matrix<double, 3, 1> obs;
                obs<< temp.dis_cur.at<float>(0),temp.dis_cur.at<float>(1),temp.dis_cur.at<float>(2);
                ORB_SLAM3::EdgeSE3RelativeXYZOnlyPose* e = new ORB_SLAM3::EdgeSE3RelativeXYZOnlyPose();

                nInitialCorrespondences++;
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
                e->setMeasurement(obs);
                e->setInformation(Eigen::Matrix3d::Identity() * temp.invSigma2);

                g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                e->setRobustKernel(rk);
                rk->setDelta(deltaMono);

                e->_point[0] = temp.dis_ori.at<float>(0);
                e->_point[1] = temp.dis_ori.at<float>(1);
                e->_point[2] = temp.dis_ori.at<float>(2);

                e->mUnstableProb = temp.unstableProb;
                optimizer.addEdge(e);

                vpEdgesMono.push_back(e);
                vnIndexEdgeMono1.push_back(temp.i);
                vnIndexEdgeMono2.push_back(temp.k);
            }
        }
    }
        
        // cv::Mat feature_img = mLastFrame.mImRGB.clone();
        // for(int i=0;i<mLastFrame.N;i++)
        // {
        //     if(!mLastFrame.mvpMapPoints[i]) continue;
        //     cv::Point kp = mLastFrame.mvKeys[i].pt;
        //     if(staticProb[i]>0.1)
        //         cv::circle(feature_img, kp, 2, cv::Scalar(0, 0, 255), staticProb[i]*10);
        //     else
        //         cv::circle(feature_img, kp, 2, cv::Scalar(0, 255, 0), 1);
        // }
        // cv::imwrite("/media/mxc/bs/SeaSLAM/result/debug/staticP.png",feature_img);
    // });
    ...
    for (size_t it = 0; it < 4; it++) {
        ...
        nBad = 0;
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ORB_SLAM3::EdgeSE3RelativeXYZOnlyPose* e = vpEdgesMono[i];

            const size_t idx1 = vnIndexEdgeMono1[i];
            const size_t idx2 = vnIndexEdgeMono2[i];

            if (pFrame->mvbOutlier[idx1]||pFrame->mvbOutlier[idx2]) {
                e->computeError();
            }
            ...
            if (chi2 > chi2Mono[it]) {
                pFrame->mvbOutlier[idx1] = true;
                pFrame->mvbOutlier[idx2] = true;
                e->setLevel(1);
                nBad++;
            } else {
                pFrame->mvbOutlier[idx1] = false;
                pFrame->mvbOutlier[idx2] = false;
                e->setLevel(0);
            }
...
void saveErr(vector<float> &errordis,int id);

int Optimizer::PoseOptimizationWithRelative(Frame* pFrame)
{
    vector<pair<int,MapPoint*>> x3D_static;
    vector<pair<float, int> > vDepthIdx;
    vDepthIdx.reserve(pFrame->N);
    // cv::Mat imgVisual = pFrame->mImRGB.clone();
   {
        int num_id=0;
        for(int i=0;i<pFrame->N;i++)
        {
    ...
    const float deltaMono = sqrt(5.991);
    const float deltaStereo = sqrt(7.815);
    vector<ORB_SLAM3::EdgeSE3RelativeXYZOnlyPose*> vpRelativeEdgesMono;
    vector<size_t> vnRelativeEdgesMono1,vnRelativeEdgesMono2;
    ...
    int outlier_map_mp = 0;
    {
        ...
        // if (!vDepthIdx.empty()) 
        //     sort(vDepthIdx.begin(), vDepthIdx.end());
        // vector<float> histDis = vector<float>(100,0);
        // vector<float> errordis;
        // cv::Mat showImg = pFrame->mImRGB.clone();
        int cnt_edge=0;
        int static_sz=x3D_static.size();
        int mono_num=30;//vpEdgesMono.size();
        static_sz = min(mono_num,static_sz);
        vector<relativeEdge> optimize_edge;
        for(int ii=0;ii<static_sz;ii++)
        {
            int i=rand()%static_sz;
            cv::Mat cur3Di = pFrame->UnprojectStereoCamera(x3D_static[i].first);
            if(cur3Di.rows==0) continue;
            cv::Mat cur3Dk ;
            cv::Mat posi=x3D_static[i].second->GetWorldPos();
            for(int j=0;j<10;j++){
                // int k = select_randomly(x3D_static.begin(), x3D_static.end())-x3D_static.begin();
                int k=rand()%x3D_static.size();
                if(k>=x3D_static.size()||k==i) continue;
                cur3Dk = pFrame->UnprojectStereoCamera(x3D_static[k].first);
                if(cur3Dk.rows==0) continue;
                cv::Mat dis_cur = cur3Di-cur3Dk;
                Eigen::Matrix<double, 3, 1> obs;
                ...
                cv::Mat dis_ori = posi-x3D_static[k].second->GetWorldPos();
                
                cv::Mat dis_temp = pFrame->UnprojectStereo(x3D_static[i].first)-pFrame->UnprojectStereo(x3D_static[k].first);
                // cv::circle(imgVisual,pFrame->mvKeys[x3D_static[k].first].pt,norm(dis_temp-dis_temp_ori)*5,cv::Scalar(0,255,0),-1);
                // int disik = int(norm(dis_temp-dis_ori)*100);
                // if(disik>20) {
                //     pFrame->mvbOutlier[x3D_static[k].first] = true;
                //     pFrame->mvbOutlier[x3D_static[i].first] = true;
                //     continue;
                // }
                float disik = norm(dis_temp-dis_ori);
                relativeEdge temp_edge;
                temp_edge.i=x3D_static[i].first;
                temp_edge.k=x3D_static[k].first;
                temp_edge.dis=disik;
                ...
                temp_edge.dis_ori=dis_ori.clone();
                temp_edge.invSigma2 = invSigma2;
                // temp_edge.unstableProb = max(1.,norm(dis_temp-dis_ori)*2);
                optimize_edge.push_back(temp_edge);
                cnt_edge++;            
            }
        }
        
        cv::Mat relError = cv::Mat::zeros(cnt_edge,1,CV_32F);
        for(int i=0;i<cnt_edge;i++)
        {
            relError.at<float>(i,0)=optimize_edge[i].dis;
        }
        if(pFrame->mnId<5)
            mWeightCalcu->calculateScale(relError);
        for(int i=0;i<cnt_edge;i++)
        {
           relativeEdge temp=optimize_edge[i];
            if(temp.dis_cur.rows==0)    continue;
            // if(temp.dis==dis1||temp.dis==dis2||temp.dis==dis3)
            {
                ...
                e->mUnstableProb = mWeightCalcu->calculateWeight(temp.dis)/2;
                optimizer.addEdge(e);

                vpRelativeEdgesMono.push_back(e);
                vnRelativeEdgesMono1.push_back(temp.i);
                vnRelativeEdgesMono2.push_back(temp.k);
            }
        }
    }
    // saveErr(errordis,pFrame->mnId);
    // if (outlier_map_mp > 0) {
    //     LOG(INFO) << "Outliers in Map of [" << pFrame->mnId << "]:" << outlier_map_mp;
    // }
    //cout << "PO: vnIndexEdgeMono.size() = " << vnIndexEdgeMono.size() << "   vnIndexEdgeRight.size() = " << vnIndexEdgeRight.size() << endl;
...
void saveErr(vector<float> &errordis,int id)
{
    fstream PLeft48("./result/errordis" + to_string(id) + ".txt", ios::out);
    if (!PLeft48.fail())
    {
        cout << "start writing PLeft48.txt" << endl;
        for (int i = 0; i < errordis.size(); i++)
        {
            PLeft48 << errordis[i] << "\t";
        }
