...
Frame::Frame(const Frame& frame)
    ...
    // , mvpClusterMapPoint(frame.mvpClusterMapPoint)
    // , mvpCluster(frame.mvpCluster)
    ...
    , mImMask(frame.mImMask.clone())
    , mImObject(frame.mImObject.clone())
    , mImMaskOld(frame.mImMaskOld.clone())
    , mImObjectOld(frame.mImObjectOld.clone())
    , mImFlow(frame.mImFlow.clone())
    , mnObject(frame.mnObject)
    , mnObjectOld(frame.mnObjectOld)
    , mbIsKeyFrame(frame.mbIsKeyFrame)
    , mbIsTracked(frame.mbIsTracked)
    , mvbKptOutliers(frame.mvbKptOutliers)
    , mvCorres(frame.mvCorres)
    , mvObjCorres(frame.mvObjCorres)
    , mvFlowNext(frame.mvFlowNext)
    , mvObjFlowNext(frame.mvObjFlowNext)
    , mvMatchId(frame.mvMatchId)
{
    ...
    , mTimeORB_Ext(0)
{
    ...
    mnId = nNextId++;
    mImGray = imLeft;
    cv::cvtColor(imLeft,mImRGB,CV_GRAY2RGB);
    // 图像金字塔的参数
    // Scale Level Info
    // 获取金字塔的层数
...
// ORB extraction
    ...
    // mvpCluster = vector<Cluster*>(N, static_cast<Cluster*>(NULL));
    // mvpClusterMapPoint = vector<MapPoint*>(N, static_cast<MapPoint*>(NULL));
...
Frame::Frame(const cv::Mat& imRGB, const cv::Mat& imGray, const cv::Mat& imDepth, const double& timeStamp, ORBextractor* extractor, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, GeometricCamera* pCamera, Frame* pPrevF, const IMU::Calib& ImuCalib)
    : mpcpi(NULL)
    , mpORBvocabulary(voc)
    ...
    , mTimeORB_Ext(0)
{
    // TODO [Semantic] for semantic segmentation
    mImRGB = imRGB;
    mImDepth = imDepth;
    mImGray = imGray;
    mbIsKeyFrame = false;
    ...
    // TODO evaluate the delay here
    //  get the latest keyframe that obtained semantic label
    size_t nLatestKeyFrameID = Semantic::GetInstance()->GetLatestSemanticKeyFrame();
    size_t delay = mnId - nLatestKeyFrameID;
    Semantic::GetInstance()->mvSemanticDelay.push_back(delay);

    // [debug]
    //Config::GetInstance()->saveImage(imRGB, "rgb", std::to_string(mnId) + ".png");
    //Config::GetInstance()->saveImage(imDepth, "depth", std::to_string(mnId) + ".png");
...
// TODO [Semantic] wait semantic result, as parameter
// Get semantic result for the first few frames, by default , just wait the first frame
    ...
    , mTimeORB_Ext(0)
{
    ...
    cv::cvtColor(imGray,mImRGB,cv::COLOR_GRAY2RGB);
    mImGray = imGray;
    // Scale Level Info
    mnScaleLevels = mpORBextractorLeft->GetLevels();
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();
...
cv::Mat Frame::UnprojectStereoCamera(const int& i)
{
...
int Frame::IsDynamicInArea(const float& x, const float& y, const float& r, const bool bRight) const
{
        ...
        for (int iy = nMinCellY; iy <= nMaxCellY; iy++) {
            ...
            for (size_t j = 0, jend = vCell.size(); j < jend; j++) {
                ...
                if (fabs(distx) < r && fabs(disty) < r){
                    if(!mvpMapPoints[vCell[j]]&&!mvpDynamicPoints[vCell[j]])
                        continue;
                    if(mvpMapPoints[vCell[j]]){
                        if(mvpMapPoints[vCell[j]]->IsDynamicMapPoint()||
                                mvpMapPoints[vCell[j]]->mbGeometryDynamic)
                        {
                            if(sqrt(distx*distx+disty*disty)<dis){
                                dis = sqrt(distx*distx+disty*disty);
                                idx=vCell[j];
                            }
                        }
                    }
                    else if(mvpDynamicPoints[vCell[j]]){
                        if(sqrt(distx*distx+disty*disty)<dis){
                                dis = sqrt(distx*distx+disty*disty);
                                idx=vCell[j];
                        }
