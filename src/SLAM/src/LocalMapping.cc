...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            if(!mpCurrentKeyFrame->checkNeighbor(kp1.pt))
            {
                continue;
            }
            MapPoint* pMP1=mpCurrentKeyFrame->mvpMapPoints[idx1];
            MapPoint* pMP2 = pKF2->mvpMapPoints[idx2];
            if(pMP1&&pMP1->IsDynamicMapPoint())
                continue;
            if(pMP2&&pMP2->IsDynamicMapPoint())
                continue;
            // if(pMP2&&!mpCurrentKeyFrame->checkMask(pMP2))
            //     continue;
