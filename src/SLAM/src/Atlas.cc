...
// void Atlas::UpdateMovingProb(KeyFrame* pKF,vector<cv::Mat> &masks)
// {
//     cv::Mat newMask = pKF->mImMask.clone();
//     cv::Mat flow = pKF->mFrame->mImMask;
//     if(flow.rows!=0)
//     {
//         flow.copyTo(newMask,(flow!=0)&(newMask==0));
//     }
//     pKF->mImMask=newMask.clone();

// }
// void Atlas::UpdateMask(cv::Mat &imMask,cv::Mat &imFlow,cv::Mat &outMask)
// {
...
//                     outMask.at<uchar>(j+flow_y,k+flow_x) = objectid;
...
//     std::vector<Cluster*>::iterator it=mvpCluster.begin();
//     for(int i=0;i<mvpCluster.size();i++)
//     {
//         if(updateFlag[i]){
//             mvpCluster[i]->updateBbox(ptlu[i],ptrd[i]);
//             it++;
//         }
//     }

// }
// void Atlas::OptimizeClusters(KeyFrame *pKF)
// {
//     if(pKF->mObjectNum==0)
//         return;
...
//                 continue;
//             int objectid = pKF->mImObject.at<uchar>(pKF->mvKeys[i].pt.y,pKF->mvKeys[i].pt.x);
//             if(objectid!=0){
//                 cv::Mat x3D = pKF->UnprojectStereo(i);
//                 if(x3D.empty())
//                     continue;
//                 MapPoint* pMP = new MapPoint(x3D,pKF,GetCurrentMap());
//                 mvpCluster[objectid-1]->AddMapPoint(i,pMP);
//                 // mvpCluster[objectid-1]->AddKeyPoint(pKF->mvKeysUn[i]);
//             }
//         }
//     }
// }

// void Atlas::UpdateClustersBbox(Frame* pFrame)
// {
//     vector<bool> updateFlag(mvpCluster.size(),false);
//     vector<cv::Point2f> ptlu(mvpCluster.size(),cv::Point2f(480.,640.));
//     vector<cv::Point2f> ptrd(mvpCluster.size(),cv::Point2f(0.,0.));
//     for(int i=0;i<pFrame->mvObjCorres.size();i++)
//     {
//         int id = pFrame->mvObjectLabel[i]-1;
//         ptlu[id].x=min(pFrame->mvObjCorres[i].pt.x,ptlu[id].x);
//         ptlu[id].y=min(pFrame->mvObjCorres[i].pt.y,ptlu[id].y);
//         ptrd[id].x=max(pFrame->mvObjCorres[i].pt.x,ptrd[id].x);
//         ptrd[id].y=max(pFrame->mvObjCorres[i].pt.y,ptrd[id].y);
//         if(!updateFlag[id])
//             updateFlag[id]=true;
//     }
//     std::vector<Cluster*>::iterator it=mvpCluster.begin();
...
//     }
// }
// void Atlas::updateCurrentCluster(Frame* pFrame)
// {

// }
// vector<Cluster*> Atlas::GetClusters()
// {
//     return mvpCluster;
// }
