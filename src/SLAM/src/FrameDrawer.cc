...
cv::Mat FrameDrawer::DrawFrame(bool bOldFeatures)
{
    ...
    } else if (state == Tracking::OK && bOldFeatures) //TRACKING
    {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            // cv::circle(im, mvOutlierKeys[i].pt, 2, cv::Scalar(0, 0, 255), -1);
        }
    ...
    // TODO [semantic] save feature result
    // Config::GetInstance()->saveImage(im, "feature", std::to_string(currentFrame.mnId) + ".png");
...
void FrameDrawer::Update(Tracking* pTracker)
{
    ...
    if (pTracker->mLastProcessedState == Tracking::NOT_INITIALIZED) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (pMP) {
                // TODO [Semantic] deal with outliers according to moving probability
