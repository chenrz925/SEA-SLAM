...
namespace ORB_SLAM3 {
    ...
    void  EdgeProjectXYZPoseOnly::linearizeOplus()
    {
    ...
    bool EdgeProjectXYZPoseOnly::read(std::istream& is)
    {
        for (int i=0; i<3; i++)
        {
            is >> _measurement[i];
    ...
    bool EdgeProjectXYZPoseOnly::write(std::ostream& os) const
    {
        for (int i=0; i<3; i++){
            os  << _measurement[i] << " ";
        }
    ...
    bool EdgeSE3RelativeXYZOnlyPose::read(std::istream& is){
        for (int i=0; i<3; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<3; i++)
    ...
    bool EdgeSE3RelativeXYZOnlyPose::write(std::ostream& os) const {
    ...
    void EdgeSE3RelativeXYZOnlyPose::linearizeOplus() {
        g2o::VertexSE3Expmap* pose = static_cast<g2o::VertexSE3Expmap *>(_vertices[0]);
        g2o::SE3Quat T(pose->estimate());
