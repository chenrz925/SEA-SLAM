...
cv::Mat Tracking::GrabImageStereo(const cv::Mat& imRectLeft, const cv::Mat& imRectRight, const double& timestamp, string filename)
{
    ...
    // future<cv::Mat> opticalFlowThread = async(launch::async, [this] {
    //     cv::Mat flowMat;
    //     if(mLastFrame.mImGray.rows==0)
    //         return flowMat;
    //     unique_lock<mutex> lock(mMutexSemanticUpdate);
    //     if(mpKFSemantic){
    //         if(mpAtlas->useKF){
    //             cv::calcOpticalFlowFarneback((mpKFSemantic->mImGray),mImGray,flowMat,0.5, 3, 20, 3, 7, 1.5, 0);
    //         }
    //         else{
    //            cv::calcOpticalFlowFarneback(mLastFrame.mImGray,mImGray,flowMat,0.5, 3, 20, 3, 7, 1.5, 0);
    //         }
    //     }  
    //     return flowMat;
    // });
    // cv::Mat geoMask;
    // future<cv::Mat> movingCheckThread = async(launch::async, [this] {
    //     cv::Mat Mask;
    //     if(mLastFrame.mImGray.rows!=0)
    //     {
    //         ProcessMovingObject(mImGray, mLastFrame.mImGray,Mask);
    //     }
    //     return Mask;
    // });
    ...
    // if(mpAtlas->mvpCluster.size()){
    //     if(!flowMat.empty()){
    //         if(mpKFSemantic){
    //             mCurrentFrame.UpdateMaskAndOpticalFlow(mpKFSemantic,mpKFSemantic->mImMask,flowMat,mLastFrame.mImFlow,mpAtlas->mvpCluster );
    //             mpAtlas->useKF=false;
    //         }
    //         else
    //             mCurrentFrame.UpdateMaskAndOpticalFlow(&mLastFrame,mLastFrame.mImMask,flowMat,mLastFrame.mImFlow,mpAtlas->mvpCluster );
        
    //         // mpAtlas->updateCurrentCluster(&mCurrentFrame);
    //     }
    // }
    Track();
    // cv::Mat flowMat = opticalFlowThread.get();
    // if(!flowMat.empty()){
    //     if(mpKFSemantic){
    //         mCurrentFrame.UpdateMask(mpKFSemantic,mpKFSemantic->mImMask,flowMat,mLastFrame.mImFlow);
    //         mpAtlas->useKF=false;
    //     }
    //     else
    //         mCurrentFrame.UpdateMask(&mLastFrame,mLastFrame.mImMask,flowMat,mLastFrame.mImFlow);
    // }
    // geoMask = movingCheckThread.get();
    // UpdateGeometryMovingProbability(geoMask);
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat& imRGB, const cv::Mat& imD, const double& timestamp, string filename)
{
    ...
    // cv::Mat imFlow = mLastFrame.mImFlow.clone();
    // cv::Mat imMaskLast = mLastFrame.mImMask.clone();    
    ...
    // Semantic::GetInstance()->InsertFrame(&mCurrentFrame);
    ...
    opticalFlowThread = async(launch::async, [this] {
        ...
        if(mpAtlas->useKF){
            if(mpKFSemantic){
                // cv::calcOpticalFlowFarneback((mpKFSemantic->mImGray),mImGray,flowMat,0.5, 3, 15, 3, 7, 1.5, cv::OPTFLOW_FARNEBACK_GAUSSIAN);
                DISoptFlow->calc(mpKFSemantic->mImGray,mImGray,flowMat);
            }
        }
        else{
            DISoptFlow->calc(mLastFrame.mImGray,mImGray,flowMat);
            // cv::calcOpticalFlowFarneback(mLastFrame.mImGray,mImGray,flowMat,0.5, 3, 15, 3, 7, 1.5, cv::OPTFLOW_FARNEBACK_GAUSSIAN);
        }
        // split(flowMat,flow_uv);
        if(mpAtlas->useKF){
            if(mpKFSemantic){
                mCurrentFrame.UpdateMask(mpKFSemantic,mpKFSemantic->mImObject,flowMat,mLastFrame.mImFlow);
            }            
            mpAtlas->useKF=false;
        }
        else if(mLastFrame.mImObject.rows!=0)
            mCurrentFrame.UpdateMask(&mLastFrame,mLastFrame.mImObject,flowMat,mLastFrame.mImFlow);
        // return flowMat;
    });
    // std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    Track();
    // mCurrentFrame.UpdatePrioriMovingProbability();
    // if(mCurrentFrame.mbIsKeyFrame)
    //     Optimizer::PoseOptimization(&mLastFrame);

    // cv::Mat pose;
    // mCurrentFrame.GetPose(pose);
    // if(pose.rows!=0)
    //     mLastFrame.SetPose(pose);
    // geoMask=movingCheckThread.get();
    // UpdateGeometryMovingProbability(geoMask);
    // std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        if (!mbOnlyTracking) {
            ...
            if (mState == OK) {
               ...
               // t0 = std::chrono::steady_clock::now();
                // opticalFlowThread.get();
                // t1 = std::chrono::steady_clock::now();
                // t_track = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t1 - t0).count();
                // cout<<"opticalFlowThread:"<<t_track<<endl;
                // opticalFlowThread.get();
                if ((mVelocity.empty() && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
                    ...
                    // t1 = std::chrono::steady_clock::now();
                    // t_track = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t1 - t0).count();
                    // cout<<"TrackReferenceKeyFrame:"<<t_track<<endl;
                    bOK = TrackReferenceKeyFrame();
                } else {
                    ...
                    // t1 = std::chrono::steady_clock::now();
                    // t_track = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t1 - t0).count();
                    // cout<<"TrackWithMotionModel:"<<t_track<<endl;
                    bOK = TrackWithMotionModel();
                    if (!bOK)
                        bOK = TrackReferenceKeyFrame();
                }
                if (!bOK) {
                    if (mCurrentFrame.mnId <= (mnLastRelocFrameId + mnFramesToResetIMU) && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)) {
                        mState = LOST;
                ...
                if (mState == RECENTLY_LOST) {
                    ...
                    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)) {
                        ...
                        if (!bOK) {
                            ...
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            LOG(INFO) << "Track Lost ...";
                            LOG(INFO) << "Id of Current tracking lost frame: " << mCurrentFrame.mnId;
                            bOK = false;
                        }
                    }
                } else if (mState == LOST) {
            ...
            if (mState == LOST) {
                ...
                // if(mpAtlas->GetCurrentMap()->GetAllClusters().size()>0)
                //     TrackClusterMapPoints();
                if (!mbVO) {
        ...
        if (!mbOnlyTracking) {
            if (bOK) {
                ...
                // t1 = std::chrono::steady_clock::now();
                // t_track = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t1 - t0).count();
                // cout<<"TrackLocalMap:"<<t_track<<endl;
                bOK = TrackLocalMap();
#ifdef SAVE_TIMES
        ...
        if (bOK || mState == RECENTLY_LOST) {
            ...
            // t1 = std::chrono::steady_clock::now();
            // t_track = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t1 - t0).count();
            // cout<<"NeedNewKeyFrame:"<<t_track<<endl;
            bool bNeedKF = NeedNewKeyFrame();
#ifdef SAVE_TIMES
            ...
            if (bNeedKF && (bOK || (mState == RECENTLY_LOST && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)))) {
                // TODO request semantic Segmentation
                // Semantic::GetInstance()->RequestSegment(mCurrentFrame.mnId, mImRGB);
                // Semantic::GetInstance()->InsertFrame(&mCurrentFrame);
                // t0 = std::chrono::steady_clock::now();
                // CreateNewKeyFrame();
                // t1 = std::chrono::steady_clock::now();
                // t_track = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t1 - t0).count();
                // cout<<"CreateNewKeyFrame:"<<t_track<<endl;
                CreateNewKeyFrame();
            }
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Bundle Adjustment
    Verbose::PrintMess("New Map created with " + to_string(mpAtlas->MapPointsInMap()) + " points", Verbose::VERBOSITY_QUIET);
    Optimizer::GlobalBundleAdjustemntForInitialMono(mpAtlas->GetCurrentMap(), 20);
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // Optimizer::PoseOptimizationWithRelative(&mCurrentFrame);
...
void Tracking::UpdateLastFrame()
{
    ...
    // // ---------------
    // // TODO Semantic: find connected keyframes that have semantic mask
    // // KeyFrame* pRef = mLastFrame.mpReferenceKF;
    // std::vector<KeyFrame*> vpLocalKeyFrames = pRef->GetBestCovisibilityKeyFrames(15);
    // int nHasSemantic = 0;
    ...
    //     nHasSemantic++;
    //
    //     PredictMovingProbability(pKFi, &mLastFrame);
    // }
    // if (nHasSemantic > 0) {
    //     LOG(INFO) << "[" << pRef->mnId << "]: "
    //               << "nHasSemantic: " << nHasSemantic;
    // }
    // // -----------
    ...
    for (size_t j = 0; j < vDepthIdx.size(); j++) {
        ...
        if (bCreateNew&&mLastFrame.checkNeighbor(i)) {
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // int nmatches = matcher.SearchByProjectionPre(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
    int nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
    
    // If few matches, uses a wider window search
    if (nmatches < 20) {
        Verbose::PrintMess("Not enough matches, wider window search!!", Verbose::VERBOSITY_NORMAL);
        LOG(INFO) << "Frame ID: " << mCurrentFrame.mnId << " Not enough matches, wider window search!!";
        LOG(INFO) << "Frame ID: " << mCurrentFrame.mnId << " matched num:" << nmatches;

        fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), static_cast<MapPoint*>(NULL));

        nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
        // nmatches = matcher.SearchByProjectionPre(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
    ...
    // Optimize frame pose witPoseOptimizationWithRelativeh all matches
    Optimizer::PoseOptimization(&mCurrentFrame);
    ...
    // int object_matched = trackObject.get();
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // TODO [Semantic] request segmentation
    ...
    if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
    {
        ...
        if (!vDepthIdx.empty()) {
            ...
            for (size_t j = 0; j < vDepthIdx.size(); j++) {
                ...
                if (bCreateNew) {
                    cv::Mat x3D;
                    if(!mCurrentFrame.checkNeighbor(i))
                        continue;
...
{
    // TODO check dynamic features using moving probability
    ...
    for (vector<KeyFrame *>::const_reverse_iterator itKF = mvpLocalKeyFrames.rbegin(), itEndKF = mvpLocalKeyFrames.rend(); itKF != itEndKF; ++itKF) {
        ...
        for (vector<MapPoint *>::const_iterator itMP = vpMPs.begin(), itEndMP = vpMPs.end(); itMP != itEndMP; itMP++) {
            ...
            if (!pMP)
                continue;
            // // TODO [Semantic] Skip outliers according Moving probability
            if (Semantic::GetInstance()->IsDynamicMapPoint(pMP)||!mCurrentFrame.checkMask(pMP)) {
                outlier_map_mp++;
                continue;
            }
    ...
    if (outlier_map_mp > 0) {
        LOG(INFO) << "Outliers in Map: " << outlier_map_mp;
    }
}
...
void Tracking::SemanticUpdateLocalPoints(KeyFrame& currentKF)
{
    int outlier_map_mp = 0;

    mvpSemanticLocalMapPoints.clear();

    int count_pts = 0;

    for (vector<KeyFrame *>::const_reverse_iterator itKF = mvpSemanticLocalKeyFrames.rbegin(), itEndKF = mvpSemanticLocalKeyFrames.rend(); itKF != itEndKF; ++itKF) {
        KeyFrame* pKF = *itKF;
        const vector<MapPoint*> vpMPs = pKF->GetMapPointMatches();
        ...
        for (vector<MapPoint *>::const_iterator itMP = vpMPs.begin(), itEndMP = vpMPs.end(); itMP != itEndMP; itMP++) {
            ...
            if (pMP->mnSemanticTrackReferenceForFrame == currentKF.mnId)
                continue;
            if (!pMP->isBad()) {
                count_pts++;
                mvpSemanticLocalMapPoints.push_back(pMP);
                pMP->mnSemanticTrackReferenceForFrame = currentKF.mnId;
            }
        }
    }
    if (outlier_map_mp > 0) {
...
// void Tracking::SemanticSearchLocalPoints(KeyFrame& currentKF)
// {
//     int nToMatch = 0;
//
//     // Project points in frame and check its visibility
//     for (vector<MapPoint *>::iterator vit = mvpSemanticLocalMapPoints.begin(), vend = mvpSemanticLocalMapPoints.end(); vit != vend; vit++) {
//         MapPoint* pMP = *vit;
//
//         if (pMP->isBad())
//             continue;
...
//             continue;
//
//         // if (currentKF.isInFrustum(pMP, 0.5)) {
//             // pMP->IncreaseVisible();
//             nToMatch++;
//         }
//         if (pMP->mbTrackInView) {
//             currentKF.mmProjectPoints[pMP->mnId] = cv::Point2f(pMP->mTrackProjX, pMP->mTrackProjY);
//         }
//     // }
//
//     if (nToMatch > 0) {
        ...
        for (int i = 0; i < mCurrentFrame.N; i++) {
            ...
            if (pMP) {
                ...
                if (!pMP->isBad()) {
                    ...
                    // if (Semantic::GetInstance()->IsDynamicMapPoint(pMP)||mCurrentFrame.checkNeighbor(i)) {
...
void Tracking::PredictSemanticMask(KeyFrame* lastKF, KeyFrame* currentKF)
{
    ...
    for (size_t i = 0; i < currentKF->N; i++) {
        ...
        if (bCreateNew) {
            ...
            pMP = new MapPoint(x3D, currentKF, mpAtlas->GetCurrentMap());
...
// void Tracking::PredictMovingProbability(KeyFrame* lastKF, Frame* currentFrame)
// {
//     cv::Mat showCurrent = currentFrame->mImRGB.clone();
//     ORBmatcher matcher(0.9, true);
// 
//     // Search by projection
//     int nmatches = 0;
...
//             // TH_HIGH = 100
//             if (bestDist <= 100) {
//                 cv::KeyPoint kp2 = currentFrame->mvKeys[i];
//                 if (lastKF->mvbKptOutliers[bestIdx2]) {
//                     currentFrame->mvbKptOutliers[i] = true;
//                     // pMP->mMovingProbability = 1;
//                     cv::circle(showCurrent, kp2.pt, 2, cv::Scalar(0, 0, 255), -1);
//                     // currentKF->EraseMapPointMatch(i);
//                 } else {
//                     currentFrame->mvbKptOutliers[i] = false;
...
//     if (!showCurrent.empty()) {
//         cv::imshow("Last Frame Feature", showCurrent);
//         cv::waitKey(1);
//     }
// }
...
void Tracking::FilterGeometryDynamic(Frame &currentFrame,Frame &refFrame,vector<int> &vnMatchLast){
    ...
    vector<cv::Point2f> nextpoint;
    vector<int> idx;
    cv::Mat imgray=currentFrame.mImGray;
    cv::Mat imGrayPre = refFrame.mImGray;
    if(imGrayPre.cols==0)
        return;
    for(int i=0;i<vnMatchLast.size();i++){
        MapPoint *pMP=currentFrame.mvpMapPoints[i];
        if(pMP){
            if(pMP->IsDynamicMapPoint())
                continue;
            cv::KeyPoint kpcur = currentFrame.mvKeysUn[i];
            cv::KeyPoint kplast = refFrame.mvKeysUn[vnMatchLast[i]];
            // int x1 = kplast.pt.x, y1 = kplast.pt.y;
            // int x2 = kpcur.pt.x, y2 = kpcur.pt.y;
            ...
            // {
                prepoint.push_back(kplast.pt);
                nextpoint.push_back(kpcur.pt);
                idx.push_back(i);
            // }
    ...
    // F-Matrix
    cv::Mat mask;
    cv::Mat F = cv::findFundamentalMat(prepoint, nextpoint, mask, cv::FM_RANSAC, 0.1, 0.99);

    for (int i = 0; i < mask.rows; i++)
    {
        if (mask.at<uchar>(i, 0) == 0){
            currentFrame.mvpMapPoints[idx[i]]->mbGeometryDynamic=true;
            currentFrame.mvpDynamicPoints[idx[i]]=currentFrame.mvpMapPoints[idx[i]];
            currentFrame.mvpMapPoints[idx[i]]=static_cast<MapPoint*>(NULL);
        }
        else
        {
            ...
            double dd = fabs(A*nextpoint[i].x + B*nextpoint[i].y + C) / sqrt(A*A + B*B); //Epipolar constraints
            if (dd <= 0.1){
                continue;
            }
            else
...
void Tracking::SetSemanticMask(KeyFrame* pKF)
{
    unique_lock<mutex> lock(mMutexSemanticUpdate);
    mImMask = pKF->mImMask;
    mpKFSemantic = pKF;
}
void Tracking::SetSemanticMask(Frame* pKF)
{
    unique_lock<mutex> lock(mMutexSemanticUpdate);
    mImMask = pKF->mImMask;
    mpFrameSemantic = pKF;
}
// void Tracking::SemanticTracking(KeyFrame* lastKF, KeyFrame* currentKF)
// {
//     // std::vector<KeyFrame*> vNeighKFs = currentKF->GetBestCovisibilityKeyFrames(10);
// 
//     // KeyFrame* lastKF;
//     // int nMaxWeight = 0;
//     // for (size_t i = 0; i < vNeighKFs.size(); i++) {
//     //     KeyFrame* pKF = vNeighKFs[i];
//     //     if (!pKF)
//     //         continue;
//     //     if (pKF->isBad())
//     //         continue;
//     //     if (pKF->mnId > currentKF->mnId)
//     //         continue;
//     //     int nWeight = currentKF->GetWeight(pKF);
//     //     if (nWeight > nMaxWeight) {
//     //         nMaxWeight = nWeight;
//     //         lastKF = pKF;
//     //     }
//     // }
//     // if (!lastKF || nMaxWeight < 20) {
//     //     LOG(WARNING) << "Cannot found last frame";
//     //     return;
//     // }
// 
//     // lastKF = currentKF->mPrevKF;
//     if (!lastKF)
//         return;
//     if (lastKF->isBad())
//         return;
//     if (!lastKF->IsSemanticReady())
//         return;
// 
//     LOG(INFO) << "Semantic track last key frame";
//     LOG(INFO) << "Last Frame ID: " << lastKF->mnId;
//     LOG(INFO) << "Current Frame ID: " << currentKF->mnId;
// 
//     // // Test
//     // cv::Mat featureImg = currentKF->mImRGB;
//     // int dynamic_mps, static_mps;
//     //
//     // // TODO remove outlier features from currentframe
//     // for (int i = 0; i < currentKF->N; i++) {
//     //     // get pixel coordinates of keypoints in original image
//     //     cv::KeyPoint kp = currentKF->mvKeys[i];
//     //     if (!currentFrame->IsInImage(kp.pt.x, kp.pt.y)) {
//     //         continue;
//     //     }
//     //     // djuge dynmamic features using Mask image
//     //     if (currentFrame->mImMask.at<uchar>((int)kp.pt.y, (int)kp.pt.x) == 255) {
//     //         // only used for tracking from motion
//     //         currentFrame->mvbKptOutliers[i] = true;
//     //     }
//     //
//     //     // remove dynamic map points
//     //     MapPoint* pMP = currentFrame->mvpMapPoints[i];
//     //     if (!pMP)
//     //         continue;
//     //     if (pMP->isBad())
...
//     //         dynamic_mps++;
//     //         pMP->mMovingProbability = 1;
//     //         // Map* globalMap = pMP->GetMap();
//     //         // globalMap->EraseMapPoint(pMP);
//     //         // cv::circle(featureImg, cv::Point((int)kp.pt.x, (int)kp.pt.y), 2, cv::Scalar(0, 0, 255), -1);
//     //     } else {
//     //         static_mps++;
//     //         pMP->mMovingProbability = 0;
//     //         // cv::circle(featureImg, cv::Point((int)kp.pt.x, (int)kp.pt.y), 2, cv::Scalar(255, 0, 0), -1);
//     //     }
//     // }
// 
//     // Config::GetInstance()->saveImage(featureImg, "debug", "semantic_" + std::to_string(currentFrame->mnId) + ".png");
// 
//     // LOG(INFO) << "Dynamic map points: " << dynamic_mps;
//     // LOG(INFO) << "Static map points: " << static_mps;
//     // dynamic_mps = 0;
//     // static_mps = 0;
// 
//     // track with motion model, to optimize camera pose
//     ORBmatcher matcher(0.9, true);
// 
//     // 设置匹配过程中的搜索半径
//     int th;
...
//         th = 7; //双目
// 
//     int nmatches = matcher.SearchByProjection(*currentKF, *lastKF, th, mSensor == System::MONOCULAR);
//     if (nmatches > 20) {
//         std::cout << "Semantic track last key frame: " << currentKF->mnId << std::endl;
//         Optimizer::PoseOptimization(currentKF, true);
//     } else {
//         LOG(INFO) << "No enough matched points: " << nmatches;
//         return;
//     }
// }
