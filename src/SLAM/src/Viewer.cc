...
namespace ORB_SLAM3 {
int Viewer::IMAGE_DELAY = 2;
std::vector<cv::Scalar> Viewer::mvColors;
    ...
    , mbStopRequested(false)
{
    ...
    if (mvColors.empty()) {
        cv::RNG rng(1);
        mvColors.emplace_back(180, 119, 31);  // blue
        mvColors.emplace_back(14, 127, 255);  // orange
        mvColors.emplace_back(44, 160, 44);   // green
        mvColors.emplace_back(40, 39, 214);   // red
        mvColors.emplace_back(189, 103, 148); // purple
        mvColors.emplace_back(75, 86, 140);   // brown
        mvColors.emplace_back(194, 119, 227); // pink
        mvColors.emplace_back(127, 127, 127); // gray
        mvColors.emplace_back(34, 189, 188);  // olive
        mvColors.emplace_back(207, 190, 23);  // cyan
        for (int i = 0; i < 1000; ++i) {
            mvColors.emplace_back(rng.uniform(0, 255),
                    rng.uniform(0, 255), rng.uniform(0, 255));
        }
    }
}
cv::Scalar Viewer::GetColor(double pos) {
    return mvColors[(int) pos % mvColors.size()];
}
void Viewer::Run()
{
    mbFinished = false;
    ...
    pangolin::Var<bool> menuShowClusters("menu.Show Clusters", true, true);
    ...
    while (1) {
        ...
        // if(menuShowClusters){
        //     mpMapDrawer->DrawMapClusters(false, menuShowClusters, false, s_cam);
        // }
        ...
        // for(auto clust:mpSystem->mpAtlas->mvpCluster){
        //     if(clust->detected)
        //         cv::rectangle(toShow,clust->mBbox,GetColor(clust->mnId));
        // }
