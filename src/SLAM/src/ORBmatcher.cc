...
int ORBmatcher::SearchByProjection(Frame& F, const vector<MapPoint*>& vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints)
{
    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
        ...
        // TODO: [semanic] skip dynamic map points
        ...
        if (pMP->mbTrackInView) {
            ...
            if (!vIndices.empty()) {
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    if (bestLevel != bestLevel2 || bestDist <= mfNNratio * bestDist2) {
                        if (pMP->GetMovingProbability() > 0.5||!F.checkMask(pMP)) {
    ...
    if (nSizeGoodMapPoints < 30) {
        ...
        for (auto it = initMatchedPoints.begin(); it != initMatchedPoints.end(); it++) {
            ...
            // cv::circle(showFeature, uv, 5, cv::Scalar(0, 255, 0), -1);
            // cnt++;
            // if(cnt==initMatchedPoints.size()/2)
            //     break;
        }
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                if (bestDist1 < TH_LOW) {
                    ...
                    if(!pKF2->checkNeighbor(pKF2->mvKeys[bestIdx2].pt))
                        continue;
                    if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2)) {
...
int ORBmatcher::Fuse(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*>& vpPoints, float th, vector<MapPoint*>& vpReplacePoint)
{
    ...
    for (int iMP = 0; iMP < nPoints; iMP++) {
        ...
        if (!pKF->IsInImage(uv.x, uv.y))
            continue;
        // if(!pKF->mFrame->mImMask.empty())
        // {
        //     if(pKF->mFrame->mImMask.at<uchar>(uv.x,uv.y)!=0){
        //         pMP->SetBadFlag();
        //         continue;
        //     }
        // }
...
int ORBmatcher::SearchByProjection(Frame& CurrentFrame, Frame& LastFrame, const float th, const bool bMono)
{
    ...
    for (int i = 0; i < HISTO_LENGTH; i++)
        rotHist[i].reserve(500);
    // vector<int> rotHistTemp[HISTO_LENGTH];
    // for (int i = 0; i < HISTO_LENGTH; i++)
    //     rotHistTemp[i].reserve(500);
        ...
        if (pMP) {
            if (!LastFrame.mvbOutlier[i] ){//&& !LastFrame.mvbKptOutliers[i]) {
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    // TODO: [semanic] skip dynamic map points
                    if (pMP->mMovingProbability > 0.5||!CurrentFrame.checkNeighbor(bestIdx2)) {
                        LastFrame.mvbOutlier[i] = true;
                        
                        // // if(bin==CurrentFrame.maxflow1||bin==CurrentFrame.maxflow2||bin==CurrentFrame.maxflow3)
                        // {
                    ...
                    else if (pMP->mMovingProbability < 0.5) {
                        nmatches++;
                        CurrentFrame.mvpMapPoints[bestIdx2]=pMP;
    ...
    if (mbCheckOrientation) {
        ...
        ComputeThreeMaxima(rotHist, HISTO_LENGTH, ind1, ind2, ind3);
        // if(!(ind1==CurrentFrame.maxflow1||ind1==CurrentFrame.maxflow2||ind1==CurrentFrame.maxflow3))
        //     ind1=-1;
        // if(!(ind2==CurrentFrame.maxflow1||ind2==CurrentFrame.maxflow2||ind2==CurrentFrame.maxflow3))
        //     ind2=-1;
        // if(!(ind3==CurrentFrame.maxflow1||ind3==CurrentFrame.maxflow2||ind3==CurrentFrame.maxflow3))
        //     ind3=-1;
        for (int i = 0; i < HISTO_LENGTH; i++) {
            if (i != ind1 && i != ind2 && i != ind3) {
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
    ...
    for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
        ...
        if (pMP) {
            ...
            if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
                ...
                if (bestDist <= ORBdist) {
                    if(!CurrentFrame.checkNeighbor(bestIdx2))
                        continue;
                    CurrentFrame.mvpMapPoints[bestIdx2] = pMP;
                    nmatches++;
...
int ORBmatcher::SearchClusterByBoW(Frame* LastFrame, Frame& F)
{
    const vector<MapPoint*> vpMapPointsKF = LastFrame->mvpClusterMapPoint;

    const DBoW2::FeatureVector& vFeatVecKF = LastFrame->mFeatVec;
    ...
    while (KFit != KFend && Fit != Fend) {
        if (KFit->first == Fit->first) {
            ...
            for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
                ...
                if (pMP->isBad())
                    continue;
                const cv::Mat& dKF = LastFrame->mDescriptors.row(realIdxKF);
                ...
                for (size_t iF = 0; iF < vIndicesF.size(); iF++) {
                    if (F.Nleft == -1) {
                        ...
                        if (F.mvpClusterMapPoint[realIdxF])
                            continue;
                ...
                if (bestDist1 <= TH_LOW) {
                    if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2)) {
                        F.mvpClusterMapPoint[bestIdxF] = pMP;
                        F.mvpCluster[bestIdxF] = pMP->mpCluster;
                        // F.mvMatchId[bestIdxF] = vIndicesKF;

                        const cv::KeyPoint& kp = (!LastFrame->mpCamera2) ? LastFrame->mvKeysUn[realIdxKF] : (realIdxKF >= LastFrame->Nleft) ? LastFrame->mvKeysRight[realIdxKF - LastFrame->Nleft] : LastFrame->mvKeys[realIdxKF];

                        if (mbCheckOrientation) {
                            cv::KeyPoint& Fkp = (!LastFrame->mpCamera2 || F.Nleft == -1) ? F.mvKeys[bestIdxF] : (bestIdxF >= F.Nleft) ? F.mvKeysRight[bestIdxF - F.Nleft] : F.mvKeys[bestIdxF];
                    ...
                    if (bestDist1R <= TH_LOW) {
                        if (static_cast<float>(bestDist1R) < mfNNratio * static_cast<float>(bestDist2R) || true) {
                            F.mvpClusterMapPoint[bestIdxFR] = pMP;
                            F.mvpCluster[bestIdxFR] = pMP->mpCluster;
                            // F.mvMatchId[bestIdxFR] = vIndicesKF;
    ...
    if (mbCheckOrientation) {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++) {
            if (i == ind1 || i == ind2 || i == ind3)
                continue;
            for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
                F.mvpClusterMapPoint[rotHist[i][j]] = static_cast<MapPoint*>(NULL);
                F.mvpCluster[rotHist[i][j]] = static_cast<Cluster*>(NULL);
                nmatches--;
            }
        }
    }
...
int ORBmatcher::SearchByProjection(Frame& CurrentFrame, Frame& LastFrame, const float th, const bool bMono,vector<float>& staticProb)
{
    std::map<int, MapPoint*> goodMatchedPoints;
    std::map<int, MapPoint*> initMatchedPoints;
        ...
        if (pMP) {
            if (!LastFrame.mvbOutlier[i] && !LastFrame.mvbKptOutliers[i]) {
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    if (pMP->mMovingProbability < 0.5) {
                        cv::Point uv2 = CurrentFrame.mvKeys[bestIdx2].pt;
                        cv::circle(showFeature, uv, 2, cv::Scalar(0, 255, 0), -1);
                        CurrentFrame.mvbOutlier[bestIdx2] = false;
                        goodMatchedPoints.insert(std::pair<int, MapPoint*>(bestIdx2, pMP));
                    }
                    if (pMP->mMovingProbability == 0.5) {
                        cv::Point uv2 = CurrentFrame.mvKeys[bestIdx2].pt;
                        cv::circle(showFeature, uv, 2, cv::Scalar(255, 0, 0), -1);
                        initMatchedPoints.insert(std::pair<int, MapPoint*>(bestIdx2, pMP));
                    }
    ...
    if (nSizeGoodMapPoints < 30) {
        ...
        // nmatches = nSizeGoodMapPoints+cnt;
        nmatches = nSizeGoodMapPoints + nSizeInitMapPoints;
    }
    LOG(INFO) << "Total Num of matched: " << nmatches;
   ...
   // }
    ...
    int ORBmatcher::SearchByProjectionPre(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        int nmatches = 0;
