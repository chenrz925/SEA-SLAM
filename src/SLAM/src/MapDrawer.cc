...
void MapDrawer::DrawMapPoints()
{
    ...
    const vector<Cluster*> vpCluster = mpAtlas->GetCurrentMap()->GetAllClusters();
    if (vpMPs.empty())
        return;
    ...
    for (set<MapPoint *>::iterator sit = spRefMPs.begin(), send = spRefMPs.end(); sit != send; sit++) {
        if ((*sit)->isBad())
            continue;
        // TODO [semantic]
        // Red: dynamic map points
        // Blue: Static Map points
        if (Semantic::GetInstance()->IsDynamicMapPoint(*sit)) {
            glColor3f(1.0, 0.0, 0.0);
        } else {
    ...
    for (auto cluster:vpCluster) {
        for(auto pMP:cluster->mspMapPoint)
        {
            if (!pMP)
                continue;
            glColor3f(mvColors[cluster->mnId%10][0],mvColors[cluster->mnId%10][1],mvColors[cluster->mnId%10][2]);
...
void MapDrawer::DrawMapClusters(bool bShowGt, bool bShowPd, bool bShowModel, pangolin::OpenGlRenderState &sCam) {
    // glMatrixMode(GL_MODELVIEW);
    // {
        for (const auto& cc : mpAtlas->GetCurrentMap()->GetAllClusters()) {
            if (!cc) { continue; }
            if(!cc->detected) continue;
            // glPushMatrix();
            // glMultMatrixf(cc->mTow.m);
            // draw axes
            glBegin(GL_LINES);
            cv::Scalar bCol = cv::Scalar(mvColors[cc->mnId%10][0],mvColors[cc->mnId%10][1],mvColors[cc->mnId%10][2]);
            if (!cc) { bCol *= 0.2; }
            glColor3d(bCol[2] , bCol[1] , bCol[0] );
            float a0 = cc->mBbox1.x, a1 = cc->mBbox1.y, a2 = cc->mBbox1.z;
            float b0 = cc->mBbox2.x, b1 = cc->mBbox2.y, b2 = cc->mBbox2.z;
            glVertex3f(a0, a1, a2); glVertex3f(a0, a1, b2); glVertex3f(a0, b1, a2); glVertex3f(a0, b1, b2);
            glVertex3f(b0, a1, a2); glVertex3f(b0, a1, b2); glVertex3f(b0, b1, a2); glVertex3f(b0, b1, b2);
            glVertex3f(a0, a1, a2); glVertex3f(a0, b1, a2); glVertex3f(a0, a1, b2); glVertex3f(a0, b1, b2);
            glVertex3f(b0, a1, a2); glVertex3f(b0, b1, a2); glVertex3f(b0, a1, b2); glVertex3f(b0, b1, b2);
            glVertex3f(a0, a1, a2); glVertex3f(b0, a1, a2); glVertex3f(a0, a1, b2); glVertex3f(b0, a1, b2);
            glVertex3f(a0, b1, a2); glVertex3f(b0, b1, a2); glVertex3f(a0, b1, b2); glVertex3f(b0, b1, b2);
            glEnd();
            // glPopMatrix();
        }
    // }
