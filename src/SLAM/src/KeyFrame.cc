    ...
    , mvbOutlier(F.mvbOutlier)
{
    ...
    mImMaskOld = F.mImMask.clone();//cv::Mat::zeros(mImRGB.size(), CV_8UC1);
    mImObjectOld = F.mImObject.clone();//cv::Mat::zeros(mImRGB.size(), CV_8UC1);
    mnObjectOld = F.mnObject;
    ...
    // TODO [semantic]
    // Config::GetInstance()->saveImage(mImRGB, "rgb", std::to_string(mnId) + ".png");
    // Config::GetInstance()->saveImage(mImDepth, "depth", std::to_string(mnId) + ".png");

    // save original features    
    if(Semantic::GetInstance()->mbSaveResult)
    {
        cv::Mat saveOriginalFeature;
        mImRGB.copyTo(saveOriginalFeature);
        for (size_t i = 0; i < mvKeys.size(); i++) {
            cv::circle(saveOriginalFeature, mvKeys[i].pt, 2, cv::Scalar(0, 255, 0), -1);
        }
        Config::GetInstance()->saveImage(saveOriginalFeature, "feature", std::to_string(mnId) + ".png");
    }
}
bool KeyFrame::checkNeighbor(cv::Point kp)
{
    if(mImMask.empty()&&mImMaskOld.empty())
        return true;
    ...
    else if(!mImMaskOld.empty()){
        for(int j=-5;j<=5;j++)
        {
            for(int i=-5;i<=5;i++)
            {
                if(kp.y+j<0||kp.y+j>mImMaskOld.rows) continue;
                if(kp.x+i<0||kp.x+i>mImMaskOld.cols) continue;
                if(mImMaskOld.at<uchar>(kp.y + j, kp.x + i)!=0)
                    return false;
            }
        }  
    }
...
void KeyFrame::UpdateBoW(KeyFrame* pKF)
{
    ...
    for (size_t i = 0; i < pKF->N; i++) {
        if (pKF->mvbKptOutliers[i]) {
            continue;
        } else {
            vDesc.push_back(mDescriptors.row(i));
        }
    }
    if (vDesc.empty()) {
        LOG(WARNING) << "Descriptor is null";
        return;
    }
    // Feature vector associate features with nodes in the 4th level (from leaves up)
    // We assume the vocabulary tree has 6 levels, change the 4 otherwise
...
int KeyFrame::IsDynamicInArea(const float& x, const float& y, const float& r, const bool bRight) const
{
            ...
            for (size_t j = 0, jend = vCell.size(); j < jend; j++) {
                ...
                if (fabs(distx) < r && fabs(disty) < r)
                    if(!mvpMapPoints[vCell[j]])
                        continue;
                    else if(mvpMapPoints[vCell[j]]->IsDynamicMapPoint()||mvpMapPoints[vCell[j]]->mbGeometryDynamic)
                    {
...
void KeyFrame::AddClusterMapPoint(int i,MapPoint* pMP)
{
    // unique_lock<std::mutex> lock(mMutexCluster);
    mvpClusterMapPoint[i]=pMP;
}
void KeyFrame::AddCluster(int i,Cluster* pCluster)
{
    // unique_lock<std::mutex> lock(mMutexCluster);
...
bool KeyFrame::checkMask(MapPoint* pMP)
{
    if(mImMask.empty())
        return true;
    cv::Mat P = pMP->GetWorldPos();
