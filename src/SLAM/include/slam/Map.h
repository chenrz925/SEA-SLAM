...
public:
    ...
    void InitDynamicVec(int n,vector<vector<int>> &bbox);
    void SetSemanticLabel(int objectid,int semanticLabel);
    void ReserveDynamicVec(int n);
    void SetDynamicPose(cv::Mat pose);
    cv::Mat GetDynamicPose();
    vector<set<MapPoint*>> GetDynamicMapPoint();
    int GetDynamicMapPointNum();
...
protected:
    ...
    // for semantic
    std::vector<std::set<MapPoint*> > mspDynamicPoint;
    std::vector<int>mvSemanticLabel;
    std::vector<std::vector<int> > mvBbox;
    cv::Mat mDynamicTcw;
public:
    // object
    std::set<Cluster*> mspClusters;
    void AddCluster(Cluster* pCluster);
    void EraseCluster(Cluster* pCluster);
    Cluster* GetCluster(int cluster_id);
