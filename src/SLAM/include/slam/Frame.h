...
public:
    ...
    int IsDynamicInArea(const float& x, const float& y, const float& r, const bool bRight) const;

    // std::map<int,int> mObjectIdMap;
...
public:
    ...
    cv::Mat mImMask,mImGray,mImFlow,mImObject;
    bool mbIsKeyFrame;
    bool mbIsTracked; // pose is estimated
    // std::vector<cv::KeyPoint> mvKeysD;
    // cv::Mat mDescriptorsD;
    // DBoW2::BowVector mBowVecD;
    // DBoW2::FeatureVector mFeatVecD;
    // save the pointer to the keyframe if current frame is keyframe
    KeyFrame* mGeneratedKeyFrame;
    int maxflow1=-1,maxflow2=-1,maxflow3=-1;
    bool checkMask(MapPoint* pMP);
    int mnObject=0;
    int mnObjectOld=0;
    cv::Mat mImMaskOld,mImObjectOld;
    // float CheckDepth(int kpid);
    ...
    std::vector<cv::Point2f> mvObjFlowNext;
    std::vector<cv::KeyPoint> mvStatKeysTmp;
    std::vector<int> mvObjectLabel;
    std::vector<cv::Point3f> vFlow_3d;
    void UpdateMaskAndOpticalFlow(KeyFrame* pKF,cv::Mat &imMask,cv::Mat &imFlow,cv::Mat &imFlowLast,vector<Cluster*> &vpCluster);

    void UpdateMaskAndOpticalFlow(Frame* pKF,cv::Mat &imMask,cv::Mat &imFlow,cv::Mat &imFlowLast,vector<Cluster*> &vpCluster);
    // void UpdatePrioriMovingProbability();
    // void ProcessMovingObject(const cv::Mat &imgray, Frame* pLastFrame);
    void UpdateMask(KeyFrame* pKF,cv::Mat imMask,cv::Mat &imFlow,cv::Mat &imFlowLast);
    void UpdateMask(Frame* pKF,cv::Mat imMask,cv::Mat &imFlow,cv::Mat &imFlowLast);
    void UpdateMask(Frame* pKF,Frame* pLastFrame,cv::Mat &imFlow,cv::Mat &imFlowLast);
    
    bool checkNeighbor(int i);
    void UpdatePrioriMovingProbability();
    cv::Mat UnprojectStereoCamera(const int& i);

    // std::mutex* mpMutexCluster;
    std::vector<Cluster*> mvpCluster;
    std::vector<MapPoint*> mvpClusterMapPoint;
    void AddClusterMapPoint(int i,MapPoint* pMP);
    void AddCluster(int i,Cluster* pCluster); 
    vector<int> mvMatchId;
};
