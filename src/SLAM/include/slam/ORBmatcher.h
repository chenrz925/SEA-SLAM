...
public:
    ...
    // int SearchByProjection(Frame& CurrentFrame, Frame& LastFrame, cv::Mat &imMask,cv::Mat &depth,cv::Mat& canny_mask,cv::Mat& wateredmask,const float th, const bool bMono);
    // int SearchByProjection(Frame& CurrentFrame, Frame& LastFrame, cv::Mat &imMask,const float th, const bool bMono);
    // int SearchDynamicByBoW(Frame* LastF, Frame& F, vector<MapPoint*>& vpDynamicMapPointMatches);
    
    // TODO [Semantic] Remove const label for LastFrame
    // int SearchObjectByProjection(Frame& CurrentFrame, Frame& LastFrame, Cluster* pCluster,const float th, const bool bMono);
    int SearchClusterByBoW(Frame* LastFrame, Frame& CurrentFrame);

    int SearchByProjection(Frame& CurrentFrame, Frame& LastFrame, const float th, const bool bMono);
    int SearchByProjection(Frame& CurrentFrame, Frame& LastFrame, const float th, const bool bMono,vector<float> &staticProb);
    int SearchByProjection(KeyFrame& CurrentFrame, KeyFrame& LastFrame, const float th, const bool bMono);
    int SearchByProjectionPre(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono);
    ...
    // int SearchDynamicByBoW(KeyFrame* pKF, Frame& F, std::vector<MapPoint*>& vpDynamicMapPointMatches);
    int SearchByProjection(Frame& CurrentFrame, Frame& LastFrame,vector<int> &vnMatchLast, const float th, const bool bMono);
