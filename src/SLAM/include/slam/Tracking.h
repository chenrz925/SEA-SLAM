...
public:
    ...
    void FilterGeometryDynamic(Frame &currentFrame,Frame &refFrame,vector<int> &vnMatchLast);
    // void SemanticSearchLocalPoints(KeyFrame& currentKF);
    // void RegistrateDynamicPoints(Frame* pFrame,vector<MapPoint*> &vpDynamicMapPointMatches,int &nmatches);
    void SetSemanticMask(Frame* pKF);
    void SetSemanticMask(KeyFrame* pKF);
    KeyFrame* mpKFSemantic=nullptr;
    Frame* mpFrameSemantic=nullptr;
...
public:
    ...
    Ptr<DenseOpticalFlow> DISoptFlow = DISOpticalFlow::create(DISOpticalFlow::PRESET_MEDIUM);
...
public:
    cv::Mat mImRight,mImDepthForMask,mImCannyMask,mImWateredMask,mImSlicMask;
    cv::Mat mImMask,mImVis,hsv;
    vector<cv::Mat> mImHist,mImROI;
    map<int,int>mObjectIdMap;
    KeyFrame* mpLastSemanticKF=nullptr;
    set<int> mvStaticObjectId;
    vector<cv::Rect> mvObjectBbox;
    vector<int> mvOrder;
    vector<float> mvMeanDepth;
    int hsize = 16;
    float hranges[2] = {0,180};
    const float* phranges = hranges;

    void TrackClusterMapPoints();
    void UpdateGeometryMovingProbability(cv::Mat &mask);
    void ProcessMovingObject(const cv::Mat &imgray, cv::Mat &imggraypre,cv::Mat &mask);
