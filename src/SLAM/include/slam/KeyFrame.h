...
namespace ORB_SLAM3
{
    ...
    public:
        ...
        bool checkNeighbor(cv::Point kp);

        // RGBD image
        cv::Mat mImRGB, mImDepth, mImLabel, mImLabelColor, mImScore;
        // mImMask_debug: before dialate
        cv::Mat mImObject,mImGray;
        // vector<cv::Mat> mvImObject;
        int mObjectNum,mnObjectOld=0;
        vector<int> mvSemanticLabel;
        vector<vector<int> > mvBbox;
        
        cv::Mat mImMask, mImMaskOld,mImObjectOld;
        int mnDynamicPoints;
        int mnStaticPoints;
        ...
        void RegistrateDynamicPoints(Map* pMap);
    ...
    public:
        ...
        std::vector<cv::Point> mBboxPoint;
